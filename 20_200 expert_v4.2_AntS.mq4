...
//|                                                    Smirnov Pavel   |
//|                                                 www.autoforex.ru   |
//| Оригинальный советник Смирного Павла доработтанный мной, очень     |
//| тчательной оптимизацией, и добавлением Функции автоматического     |
//| размера лота. И увеличения лота для покрытия убытков. Советник     |
//| после доработки ведёт себя очень неплохо. Но всёравно остаётся     |
//| крайне не продуктивным. В ручную можно зарабатывать больше.        |
//| Проверено на реале. Работает также как и на тесте. Рекомендуется   |
//| Для депозита от 10000$. На меньших слишком маленькая прибыльность. |
//| Только для EUR/USD на часовом графике !!!!                    AntS |
//+--------------------------------------------------------------------+

#property copyright "Smirnov Pavel"
#property link      "www.autoforex.ru"

 int TakeProfit_L = 39; // Уровень тейкпрофит в пунктах
 int StopLoss_L = 147;  // уровень стоплосс в пунктах
 int TakeProfit_S = 32; // Уровень тейкпрофит в пунктах
 int StopLoss_S = 267;  // уровень стоплосс в пунктах
...
extern double lot = 0.01;      // Размер лота
 int Orders=1;          // максимальное количество одновременно открытых позиций
 int MaxOpenTime=504;
 int BigLotSize = 6;    // На сколько умножается размер лота в Биг лоте
